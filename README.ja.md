...
`lfq` パッケージは、ロックフリーおよびウェイトフリーな有界 FIFO キューを提供します。特定のプロデューサー/コンシューマーパターン（SPSC、MPSC、SPMC、MPMC）に最適化されており、高競合下でも予測可能でゼロアロケーションなスケーリングを保証します。

### 理論と背景

`lfq` は、従来のミューテックスや CAS（Compare-And-Swap）ループの限界を克服するために、基礎的な並行処理研究に基づいて構築されています：

- **Lamport リングバッファ (1977)**：単一プロデューサー/単一コンシューマー（SPSC）パスを駆動し、コストの高いアトミックな Read-Modify-Write 命令なしで厳密な $O(1)$ ウェイトフリーレイテンシを提供します。
- **Scalable Circular Queue（SCQ、2019）**：マルチパーティ（MPSC、SPMC、MPMC）パスは Ruslan Nikolaev の SCQ アルゴリズムに基づいて実装されています。CAS ループの代わりにハードウェア Fetch-And-Add（FAA）命令を活用することで、`lfq` は競合ボトルネックや ABA 問題を本質的に回避し、外部の安全なメモリ回収機構（ハザードポインタなど）を必要としない独立したキューとして動作します。

### `iox` ノンブロッキング I/O スタック

`lfq` は `iox` ベースのノンブロッキング I/O スタック向けに設計されています。暗黙的なランタイムブロッキングを避け、明示的なバックプレッシャーを採用しています。キューが満杯または空の場合、即座に `ErrWouldBlock` を返します。この簡潔なエラーモデルは、イベントループや呼び出し側のバックオフプリミティブ（`iox.Backoff` など）と完全に統合され、予測可能なトラフィック管理とサブマイクロ秒のレイテンシ境界を実現します。
...
| **MPSC** | 複数プロデューサー単一コンシューマー | ロックフリー（デキューはウェイトフリー） | イベント集約、ロギング |
| **SPMC** | 単一プロデューサー複数コンシューマー | ロックフリー（エンキューはウェイトフリー） | タスク分散 |
...
デフォルトでは、マルチアクセスキューは SCQ（スケーラブル循環キュー）アルゴリズムを FAA（Fetch-And-Add）命令で実装しています。FAA は位置カウンターを盲目的にインクリメントするため、容量 n に対して 2n の物理スロットが必要ですが、高競合下では CAS ベースの代替案よりも優れたスケーラビリティを発揮します。

- **トレードオフ**: 名目容量 `n` に対して物理スロット `2n` が必要です。
- **一時容量**: `P` 個の並行プロデューサーがいる場合、バックプレッシャーが効く前に `Cap()` を最大 `P-1` 個一時的に超えてエンキューされることがあります。
...
Indirect および Ptr キューバリアント（非 SPSC のうち Compact Indirect を除く）は、シーケンス番号とペイロードを単一の 128 ビットアトミック操作にパックします。これによりキャッシュライン競合が軽減され、高並行性下でのスループットが向上します。

ポインターを 128 ビットスロットに符号化する Ptr バリアント（FAA および PtrSeq バリアント）では、エンキューしたオブジェクトがデキューされる（または消費が確定する）まで、呼び出し側が型付き Go 参照を保持してください。キュー内のポインタービット列だけを GC 到達性の根として扱わないでください。
...
以下のパターンは、`lfq` を並行システムに統合する方法を示しています。ホットパスでのメモリ割り当てを避け、適切なキューのバリエーションを利用することで、レイテンシを大幅に削減できます。

### バッファプール

ゼロアロケーション I/O の一般的なパターンは、バッファのプールを事前割り当てし、ウェイトフリーな SPSC キューを使用して利用可能なインデックスを追跡することです。これにより、GC の圧力が完全に排除されます。
...
FAA ベースの多コンシューマーキュー（MPMC、SPMC）にはライブロック防止のための閾値メカニズムがあります。MPSC も `Drainer` を実装しますが、用途はシャットダウン通知のみです（`Dequeue` の閾値スキップは行いません）。プロデューサーがコンシューマーより先に終了するグレースフルシャットダウンでは、`Drainer` インターフェースを使用します：
...
// コンシューマーは残りの全アイテムをドレインできる。
// MPMC/SPMC では Drain により閾値起因の早期終了を回避できる。
for {
    item, err := q.Dequeue()
    if err != nil {
        break // キューが空
...
`Drain()` はヒントです — 呼び出し元は以降 `Enqueue()` を呼び出さないことを保証する必要があります。MPMC/SPMC では `Drain()` により `Dequeue` の閾値起因の早期終了を回避できます。MPSC では `Drain()` は終了通知のみです。SPSC キューは `Drainer` を実装しません。型アサーションがこのケースを自然に処理します。
...
最小容量は `2` です。`capacity < 2` の場合、コンストラクタは panic します。
